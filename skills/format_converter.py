...
def format_converter(
    ...
    if len(text) > MAX_TEXT_CHARS:
        raise ValueError(f"text exceeds limit: {len(text)} chars > {MAX_TEXT_CHARS}")
    ...
    if target == "markdown":
        ...
        try:
            ...
            except ValueError as kv_exc:
                raise ValueError("text is neither valid JSON nor key-value lines") from kv_exc
