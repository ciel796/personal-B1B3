...
B4_DEMO_OUTPUTS_DIR = AGENT_DIR / "module_demos" / "outputs" / "b4"
B4_FIXED_EVAL_DIRS = {
    "prompt_json": B4_DEMO_OUTPUTS_DIR / "20260714_060514_828439",
    "native_tools": B4_DEMO_OUTPUTS_DIR / "20260714_070554_649544",
    "adaptive": B4_DEMO_OUTPUTS_DIR / "20260714_080648_693065",
}
...
def _primary_failure_reason_from_counts(*, total: int, success: int, path: int, args: int, hard: int) -> str:
    if total <= 0 or success >= total:
        return "strict_success"
    mixed = max(0, total - success - path - args - hard)
    reason_counts = {
        "path_deviation": path,
        "args_incomplete": args,
        "hard_failure": hard,
        "mixed_issue": mixed,
    }
...
def _summarize_b4_eval_rows(rows: list[dict[str, str]]) -> list[dict[str, object]]:
    grouped: dict[tuple[str, str, str], list[dict[str, str]]] = {}
    for row in rows:
        profile = str(row.get("model_profile") or "default").strip() or "default"
        mode = str(row.get("mode") or "unknown").strip() or "unknown"
        scope = str(row.get("scope") or "unknown").strip() or "unknown"
        grouped.setdefault((profile, mode, scope), []).append(row)

    summaries: list[dict[str, object]] = []
    for (profile, mode, scope), items in grouped.items():
        total = len(items)
        success_count = sum(1 for item in items if _parse_boolish(item.get("success")))
        structured_count = sum(1 for item in items if _parse_boolish(item.get("structured_output_success")))
        tool_match_count = sum(1 for item in items if _parse_boolish(item.get("tool_match")))
        args_complete_count = sum(1 for item in items if _parse_boolish(item.get("args_complete")))
        usable_count = sum(
            1
            for item in items
            if _parse_boolish(item.get("structured_output_success"))
            and not str(item.get("error_type") or "").strip()
        )
        path_count = sum(
            1
            for item in items
            if _parse_boolish(item.get("structured_output_success"))
            and _parse_boolish(item.get("args_complete"))
            and not _parse_boolish(item.get("tool_match"))
        )
        args_issue_count = sum(
            1
            for item in items
            if _parse_boolish(item.get("structured_output_success"))
            and _parse_boolish(item.get("tool_match"))
            and not _parse_boolish(item.get("args_complete"))
        )
        hard_failure_count = sum(1 for item in items if not _parse_boolish(item.get("structured_output_success")))

        input_values = [_parse_intish(item.get("input_tokens")) for item in items]
        output_values = [_parse_intish(item.get("output_tokens")) for item in items]
        elapsed_values = [_parse_floatish(item.get("elapsed_seconds")) for item in items]
        input_values = [value for value in input_values if value is not None]
        ...
        summaries.append(
            {
                ...
                "usable_result_rate": _rate(usable_count, total),
                "tool_match_rate": _rate(tool_match_count, total),
                "structured_output_rate": _rate(structured_count, total),
                "args_complete_rate": _rate(args_complete_count, total),
                "path_deviation_rate": _rate(path_count, total),
                "args_issue_rate": _rate(args_issue_count, total),
                "hard_failure_rate": _rate(hard_failure_count, total),
                "primary_failure_reason": _primary_failure_reason_from_counts(
                    total=total,
                    success=success_count,
                    path=path_count,
                    args=args_issue_count,
                    hard=hard_failure_count,
                ),
    ...
    summaries.sort(key=lambda item: (str(item.get("profile") or ""), str(item.get("scope") or ""), str(item.get("mode") or "")))
    return summaries


def _scope_label(scope: str) -> str:
    return "单轮" if scope == "single_turn" else "多轮/自适应"


def _normalize_eval_case(row: dict[str, object], *, source_mode: str, scope: str, source_dir: Path) -> dict[str, object]:
    return {
        "case_id": str(row.get("case_id") or "").strip(),
        "title": str(row.get("title") or "").strip(),
        "category": str(row.get("category") or "").strip(),
        "mode": str(row.get("mode") or source_mode).strip(),
        "scope": scope,
        "scope_label": _scope_label(scope),
        "source_mode": source_mode,
        "source_dir": str(source_dir),
        "model_profile": str(row.get("model_profile") or "").strip(),
        "model_name": str(row.get("model_name") or "").strip(),
        "expected_tools": str(row.get("expected_tools") or "").strip(),
        "actual_tools": str(row.get("actual_tools") or "").strip(),
        "tool_match": bool(row.get("tool_match")),
        "structured_output_success": bool(row.get("structured_output_success")),
        "args_complete": bool(row.get("args_complete")),
...
def _load_fixed_b4_eval_payload() -> dict[str, object]:
    ...
    for mode_name, directory in B4_FIXED_EVAL_DIRS.items():
        summary_path = directory / "eval_summary.json"
        payload = _read_json_file_if_exists(summary_path)
        if not isinstance(payload, dict):
            continue
        available_sources[mode_name] = {
            "dir": str(directory),
            "summary_path": str(summary_path),
            "selected_mode": str(payload.get("selected_mode") or mode_name),
        }
        for value in payload.get("profiles") or []:
            profiles.add(str(value))
        for value in payload.get("modes") or []:
            modes.add(str(value))
        for scope in ("single_turn", "multi_turn"):
            section = payload.get(scope)
            ...
            for row in rows:
                if not isinstance(row, dict):
                    continue
                normalized = _normalize_eval_case(row, source_mode=mode_name, scope=scope, source_dir=directory)
    ...
    summary_items = _summarize_b4_eval_rows(all_rows)
    best_item = max(
        summary_items,
        key=lambda item: (
            float(item.get("success_rate") or 0.0),
            float(item.get("usable_result_rate") or 0.0),
            -float(item.get("avg_elapsed_seconds") or 0.0),
        ),
        default=None,
    )

    all_cases.sort(key=lambda item: (str(item.get("model_profile") or ""), str(item.get("scope") or ""), str(item.get("mode") or ""), str(item.get("case_id") or "")))
    return {
        "exists": True,
        "source_type": "fixed_mode_dirs",
        "source_dirs": available_sources,
        ...
        "best_item": best_item,
        "notes": [
            "严格成功率会把工具路径偏离、参数不完整以及结构化失败都计为失败。",
            "结果可用率更接近“答案大体可解释、流程未硬失败”的口径。",
        ],
    }


def _load_b4_eval_payload() -> dict[str, object]:
    return _load_fixed_b4_eval_payload()
...
class AgentStudioHandler(SimpleHTTPRequestHandler):
    ...
    def _api_b4_eval_latest(self) -> None:
        payload = _load_b4_eval_payload()
        payload["status"] = "ok"
        _json_response(self, payload)
